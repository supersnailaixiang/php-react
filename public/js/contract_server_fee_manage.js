...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["状态", "公司名称", "店铺名称", "卖家账号", "预计收款时间", "应收金额", "实际收款时间", "实施离开现场时间", "结项时间", "到款账号", "销售", "实施", "创建人", "创建时间"];

	var indexs = ["collect_status", "company_name", "shop_name", "seller_account", "expect_collect_time", "collect_amount", "collect_time", "leave_scene_time", "end_project_time", "collect_account", "sale_name", "imp_name", "creator_name", "created"]; //相当于sql的表头

	var table_rows = result;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "修改", "class2": "btn_chg", "target": "#payment_chg", "name": "" }, { "title": "流失", "class2": "btn_run_off", "target": "#contract_payment_run_off", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=ContractController&act=getContractServerFeeManage";

	var tabs = [{ name: "事件", url: "?controller=EventController&act=getEventListByConractID" }];
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=ContractController&act=contractServerFeeManage&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=ContractController&act=contractServerFeeManage&" + page_data);

...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["公司名称", "店铺名称", "合同编号", "卖家账号", "实施离开现场时间", "销售", "结项时间", "收款时间", "服务费赠送"];

	var indexs = ["company_name", "shop_name", "contract_no", "seller_account", "leave_scene_time", "sale_name", "end_project_time", "payment_time", "server_free"]; //相当于sql的表头
	...
	var operates = []; // 需要输入
	var page_turn = page;
	var url = "?controller=ContractController&act=getContractFinancialState";
	            ...
	            React.createElement(
	                ...
	                React.createElement(
	                    ...
	                    React.createElement(
	                        'td',
	                        null,
	                        React.createElement('input', { type: 'date', name: 'leave_time_begin' })
	                    ),
	                    React.createElement(
	                        'td',
	                        null,
	                        React.createElement('input', { type: 'date', name: 'leave_time_end' })
	                    ),
	                    React.createElement(
	                        'td',
	                        null,
	...
	var CustomerIntentionList = React.createClass({
	    ...
	    onSearchSubmit: function onSearchSubmit(data) {
	        $.ajax({
	            ...
	            success: function (result) {
	                ...
	                window.history.pushState(null, null, "?controller=ContractController&act=contractFinancialState&" + data);
	    ...
	    handleTablePage: function handleTablePage(page) {
	        ...
	        $.ajax({
	            ...
	            data: page_data,
	            success: function (result) {
	                window.history.pushState(null, null, "?controller=ContractController&act=contractFinancialState&" + page_data);
	                this.setState({ rows: result.result, page: result.page });
	            }.bind(this),
	            error: function (xhr, status, err) {

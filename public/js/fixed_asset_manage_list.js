...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["使用者", "部门", "备注", "产品名称", '进价', '购买日期', '品牌系列', '型号', 'SNID', 'S/N', 'LanMac', 'WLan Mac'];

	var indexs = ["operator_name", "dept_name", "remark", "product_name", "price", "buy_date", "brand", "model", "sn_id", 'sn', "lan_mac", "wlan_mac"]; //相当于sql的表头

	var heads_temp = ["姓名", "部门", "使用日期", "补助金", '调换原因', '调换日期', '备注'];

	var indexs_temp = ["operator_name", "dept_name", "user_date", "subsidy", "exchange_reason", "exchange_date", "remark"]; //相当于sql的表头

	var table_rows = result;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "修改", "class2": "btn_chg", "target": "#fixed_asset_update_list", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=UserController&act=getFixedAssetManageList";

	var tabs = [{ name: "日志", url: "?controller=UserController&act=getFixedAssetManageLog" }];
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement('input', { type: 'text', name: 'sn_id' })
						),
						React.createElement(
							'td',
							null,
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement(SelectComponent, { multiple: false, url: '?controller=UserController&act=getUserAll', name: 'users' })
						),
						React.createElement(
							'td',
							null,
						...
						React.createElement(
							'td',
							null,
							React.createElement(SelectComponent, { multiple: false, url: '?controller=DictDataController&act=getDictData&type=19', name: 'product_id' })
						)
					)
				)
			);
	...
	var SearchButton = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#new_fixed_asset_list' },
					'新建固定资产'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#new_product_list' },
					'新建产品'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#check_product_list' },
					'查看产品'
				)
			);
		}
	});
	//	<button data-toggle="modal"  data-target="#fixed_asset_in_stock">入库</button>
	//	<button data-toggle="modal"  data-target="#fixed_asset_out_stock">领取</button>
	//	<button data-toggle="modal"  data-target="#fixed_asset_del">删除</button>
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					window.history.pushState(null, null, "?controller=UserController&act=fixedAssetManageList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=UserController&act=fixedAssetManageList&" + page_data);

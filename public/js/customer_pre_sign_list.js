...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["销售1", "销售2", "预估金额", "公司名称", "店铺名称", "所属行业", "所属类目", "所在省", "所在市", "店铺规模", "客户预算", "客户来源", "现用软件", "提供者", "创建者", "电话销售", "修改时间", "创建时间"];

	var indexs = ["sale_user_name_1", "sale_user_name_2", "pre_sign_amount", "company_name", "shop_name", "industry_name", "cate_name", "province_name", "city_name", "shop_scale", "customer_budget", "source_name", "soft_name", "supplier_name", "creator_name", "tel_name", "modified", "created"]; //相当于sql的表头

	var table_rows = result;
	var user_flags = flags;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "修改客户", "class2": "btn_chg", "target": "#customer_info_chg", "name": "" }, { "title": "添加事件", "class2": "add_event", "target": "#add_event_list", "name": "" }, { "title": "添加任务", "class2": "add_task", "target": "#add_task_list", "name": "" }, { "title": "查看私密信息", "class2": "imporant_message", "target": "#check_important_message", "name": "" }, { "title": "设置回访", "class2": "set_visit", "target": "#set_visit_list", "name": "" }, { "title": "开启抢单", "class2": "grap_customer", "target": "", "name": "" }, { "title": "分配销售", "class2": "transfer_sale", "target": "#transfer_sales_list", "name": "" }, { "title": "客户成交", "class2": "create_survey", "target": "#create_survey_list", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=CustomerProcessController&act=getCustomerPreSignList";
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement('input', { type: 'text', name: 'tel' })
						),
						React.createElement(
							'td',
							null,
	...
	var SearchButton = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#add_flag' },
					'添加标记'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#color_flag_show' },
					'查看标记'
				),
				'设置标记:',
				React.createElement(
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=CustomerProcessController&act=customerPreSignList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=CustomerProcessController&act=customerPreSignList&" + page_data);

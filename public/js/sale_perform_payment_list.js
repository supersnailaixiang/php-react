...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["到款时间", "到款类型", "到款金额", "公司名称", "店铺名称", "到款账户", "销售1", "销售2", "提供者", "客户创建者", "电话销售", "客户来源", "软件版本", "创建时间", "收款汇总"];

	var indexs = ["collect_time", "payment_name", "payment_amount", "company_name", "shop_name", "collect_account", "sale_user_name1", "sale_user_name2",, "supplier_name", "customer_creator_name", "tel_name", "source_name", "order_name", "created", "payment_info"]; //相当于sql的表头
	...
	var page_turn = page;
	//var sum = sum;
	var url = "?controller=UserPerformController&act=getSalePerformPaymentList";
	...
	var SearchFormComponent = React.createClass({
		...
		render: function render() {
			return React.createElement(
				'form',
				{ ref: this.props.name, style: { padding: '5px' }, key: 'serach_form_list', id: 'search_form' },
				React.createElement(
					'table',
					null,
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement(SelectComponent, { name: 'payment_id', id: 'payment_id', multiple: false, url: '?controller=DictDataController&act=getDictData&type=12&index=1' })
						),
						React.createElement(
							'td',
							null,
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement(SelectComponent, { multiple: false, url: '?controller=DictDataController&act=getDictData&type=1', id: 'search_province', name: 'province_id' })
						),
						React.createElement(
							'td',
							null,
						...
						React.createElement(
							'td',
							null,
							React.createElement('select', { name: 'city_id', id: 'search_city' })
						)
					)
				)
			);
			...
			this.props.indexs.forEach(function (index) {
				...
				if (index == "collect_voucher") {
					...
					tds.push(React.createElement(
						...
						null,
						' ',
						React.createElement('img', { src: this.props.row[index] })
					));
				} else {
					tds.push(React.createElement(
						'td',
	...
	var CustomerIntentionList = React.createClass({
		...
		getInitialState: function getInitialState() {
			return { rows: table_rows, search_data: '', page: page_turn, sum: sum };
		},
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=UserPerformController&act=salePerformPaymentList&" + data);
					this.setState({ rows: result.result, search_data: data, page: result.page, sum: result.sum });
				}.bind(this),
				error: function (xhr, status, err) {
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=UserPerformController&act=salePerformPaymentList&" + page_data);
					this.setState({ rows: result.result, page: result.page, sum: result.sum });
				}.bind(this),
				error: function (xhr, status, err) {

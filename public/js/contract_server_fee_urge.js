...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["状态", "公司名称", "店铺名称", "卖家账号", "预计收款时间", "应收金额", "实际收款时间", "实施离开现场时间", "结项时间", "到款账号", "销售", "实施", "创建人", "备注类型", "创建时间"];

	var indexs = ["collect_status", "company_name", "shop_name", "seller_account", "expect_collect_time", "collect_amount", "collect_time", "leave_scene_time", "end_project_time", "collect_account", "sale_name", "imp_name", "creator_name", "remark_type", "created"]; //相当于sql的表头

	var table_rows = result;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "查看赠送", "class2": "btn_check_send", "target": "#contract_send_list", "name": "" }, { "title": "收款", "class2": "btn_collect", "target": "#contract_payment_collect", "name": "" }, { "title": "流失", "class2": "btn_run_off", "target": "#contract_payment_run_off", "name": "" }, { "title": "查看私密信息", "class2": "btn_check_message", "target": "#contract_check_message", "name": "" }, { "title": "添加备注", "class2": "btn_add_remark", "target": "#contract_add_remark", "name": "" }, { "title": "重新递交", "class2": "contract_payment_resend", "target": "", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=ContractController&act=getContractServerFeeUrge";

	var tabs = [{ name: "日志", url: "?controller=ContractController&act=contractPaymentLogByID" }, { name: "便签", url: "?controller=CustomerController&act=getLabelListBySellerID&type=2" }, { name: "回访事件", url: "?controller=CustomerProcessController&act=customerVisitEventList&type=1" }];
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement('input', { type: 'date', name: 'begin_expect' })
						),
						React.createElement(
							'td',
							null,
							React.createElement('input', { type: 'date', name: 'end_expect' })
						),
						React.createElement(
							'td',
							null,
						...
						React.createElement(
							'td',
							null,
							React.createElement('input', { type: 'date', name: 'begin_collect' })
						),
						React.createElement(
							'td',
							null,
							React.createElement('input', { type: 'date', name: 'end_collect' })
						),
						React.createElement(
							'td',
							null,
	...
	var TableCommonRowComponent = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				'tr',
				{ onClick: this.onClickRow, 'data-id': this.props.row.rec_id, 'data-seller_id': this.props.row.seller_id, 'data-province': this.props.row.province_id },
	...
	var SearchButton = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#add_run_off_list' },
					'添加流失类型'
				)
			);
		}
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=ContractController&act=contractServerFeeUrge&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=ContractController&act=contractServerFeeUrge&" + page_data);

...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["姓名", "部门", "使用日期", "补助金", '调换原因', '调换日期', '备注'];

	var indexs = ["operator_name", "dept_name", "user_date", "subsidy", "exchange_reason", "exchange_date", "remark"]; //相当于sql的表头

	var is_company_temp = is_company;
	if (!is_company_temp) {
		heads = heads_temp;
		indexs = indexs_temp;
	}
	...
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "修改", "class2": "btn_chg", "target": "#fixed_asset_update_list", "name": "" }, { "title": "删除", "class2": "btn_dele", "target": "", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=UserController&act=getFixedAssetPersonalManageList";
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					window.history.pushState(null, null, "?controller=UserController&act=fixedAssetPersonalManageList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=UserController&act=fixedAssetPersonalManageList&" + page_data);

...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["客户级别", "公司名称", "店铺名称", "状态", "卖家账号", "销售1", "销售2", "实施", "订购软件", '是否独享', "客户来源", "成交时间", "创建时间"];
	var indexs = ["implement_customer_level", "company_name", "shop_name", "status", "seller_account", "sale_user_name1", "sale_user_name2", "imp_user_name", "order_name", "is_share", "source_name", "succ_time", "created"]; //相当于sql的表头
	...
	var operates = []; // 需要输入
	var page_turn = page;
	var url = "?controller=CustomerController&act=getCustomerQueryList";

	var tabs = [{ name: "附加合同", url: "?controller=ContractController&act=getContractAddList" }, { name: "跟进记录", url: "?controller=EventController&act=getSuperEventListByID" }, { name: "内部便签", url: "?controller=CustomerController&act=getSuperLabelListBySellerID&type=1" }, { name: "服务费收取记录", url: "?controller=ContractController&act=getServerListBySellerID" }, { name: "服务费变更记录", url: "?controller=ContractController&act=getServerChgLogBySellerID" }, { name: "普通收款记录", url: "?controller=ContractController&act=getCommonPaymentBySellerID" }, { name: "任务", url: "?controller=TaskController&act=getSuperTaskListBySellerID" }, { name: "操作日志", url: "?controller=CustomerProcessController&act=customerSupperLogList" }];
	...
	var SearchButton = React.createClass({
	  ...
	  render: function render() {
	    ...
	    return React.createElement(
	      ...
	      React.createElement(
	        'button',
	        { 'data-toggle': 'modal', 'data-target': '#contract_check_list' },
	        '销售合同'
	      )
	    );
	  }
	...
	var CustomerIntentionList = React.createClass({
	  ...
	  onSearchSubmit: function onSearchSubmit(data) {
	    $.ajax({
	      ...
	      success: function (result) {
	        ...
	        window.history.pushState(null, null, "?controller=CustomerController&act=customerQueryList&" + data);
	  ...
	  handleTablePage: function handleTablePage(page) {
	    ...
	    $.ajax({
	      ...
	      data: page_data,
	      success: function (result) {
	        window.history.pushState(null, null, "?controller=CustomerController&act=customerQueryList&" + page_data);

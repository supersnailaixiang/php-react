...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["到款时间", "公司名称", "店铺名称", "到款账户", "汇款凭证", "合同编号", "到款金额", "合同金额", "未收金额", "开发票", "销售1", "销售2", "提供者", "客户创建者", "电话销售", "客户来源", "软件版本", "备注", "创建人", "所属大区", "创建时间"];

	var indexs = ["collect_time", "company_name", "shop_name", "collect_account", "collect_voucher", "contract_no", "collect_amount", "contract_amount", "contract_no_paid", "is_invoice", "sale_user_name1", "sale_user_name2", "supplier_name", "customer_creator_name", "tel_name", "source_name", "order_name", "remark", "creator_name", "region_name", "created"]; //相当于sql的表头

	var table_rows = result;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "查看合同", "class2": "check_contract", "target": "", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=ContractController&act=getContractPaymentAlConfirmList";
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, " ?controller=ContractController&act=contractPaymentAlConfirmList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, " ?controller=ContractController&act=contractPaymentAlConfirmList&" + page_data);

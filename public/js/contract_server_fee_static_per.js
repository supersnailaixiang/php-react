...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["日期", "应续费客户数", "已续费客户数", "未续费客户数", "流失客户数", "实际续费客户数"];

	var indexs = ["month", "all_collect", "yes_collect", "no_collect", "run_off", "true_collect"]; //相当于sql的表头

	var table_rows = result;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "未续费客户详情", "class2": "btn_chg", "target": "#customer_info_chg", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=ContractController&act=getContractServerFreeStatic&type=per";
	...
	var CustomerIntentionList = React.createClass({
	    ...
	    onSearchSubmit: function onSearchSubmit(data) {
	        $.ajax({
	            ...
	            success: function (result) {
	                ...
	                window.history.pushState(null, null, "?controller=ContractController&act=getContractServerFreeStatic&" + data);
	    ...
	    handleTablePage: function handleTablePage(page) {
	        ...
	        $.ajax({
	            ...
	            data: page_data,
	            success: function (result) {
	                window.history.pushState(null, null, "?controller=ContractController&act=getContractServerFreeStatic&" + page_data);

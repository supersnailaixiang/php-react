...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["销售1", "销售2", "公司名称", "店铺名称", "所属行业", "所属类目", "所在省", "所在市", "店铺规模", "客户预算", "客户来源", "现用软件", "提供者", "创建者", "电话销售", "修改时间", "创建时间"];

	var indexs = ["sale_user_name_1", "sale_user_name_2", "company_name", "shop_name", "industry_name", "cate_name", "province_name", "city_name", "shop_scale", "customer_budget", "source_name", "soft_name", "supplier_name", "creator_name", "tel_name", "modified", "created"]; //相当于sql的表头

	var table_rows = result;

	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "抢单", "class2": "btn_grap", "target": "", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=CustomerProcessController&act=getCustomerGrapList";
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=CustomerProcessController&act=customerGarpList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=CustomerProcessController&act=customerGrapList&" + page_data);

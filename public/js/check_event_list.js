...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["公司名称", "事件创建人", "事件内容", "创建时间"];

	var indexs = ["company_name", "user_name", "content", "created"]; //相当于sql的表头

	var table_rows = result;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "查看公司所有事件", "class2": "check_all_event", "target": "check_all_event_list", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=EventController&act=getCheckEventList";

	var tabs = [{ name: "客户相关事件", url: "?controller=EventController&act=getCustomerEventList" }];
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					window.history.pushState(null, null, "?controller=EventController&act=checkEventList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=EventController&act=checkEventList&" + page_data);

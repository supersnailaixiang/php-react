...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["卖家账号", "公司名称", "店铺名称", "危险客户", "批次", "回访状态", "销售", "实施", "本次回访人", "回访时间", "实施大区"];

	var indexs = ["seller_account", "company_name", "shop_name", "dangerous_customer", "bacth_name", "status", "sale_man", "imp_man", "user_name", "visit_date", "region_name"]; //相当于sql的表头

	var table_rows = result;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "添加事件", "class2": "add_event", "target": "#add_event_list", "name": "" }, { "title": "添加任务", "class2": "add_task", "target": "#add_task_list", "name": "" }, { "title": "回访失败", "class2": "fail_visit", "target": "", "name": "" }, { "title": "再次回访", "class2": "re_visit", "target": "#add_re_visit", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=CustomerProcessController&act=getCustomerVisitProcessList";

	var tabs = [{ name: "日志", url: "?controller=CustomerProcessController&act=customerVisitLog" }, { name: "回访记录", url: "?controller=CustomerProcessController&act=customerVisitEventList" }, { name: "任务", url: "?controller=CustomerProcessController&act=customerVisitTaskList" }];
	...
	var SelectComponent = React.createClass({
		...
		render: function render() {
			$.ajax({
				...
				success: function (data) {
					rows = [];
					if (this.props.url == '?controller=CustomerProcessController&act=getCustomerSuccessBackBacth&type=1') {
	...
	/*var SelectComponent = React.createClass({	 
			onChangeValue:function(){
		     ...
		     render: function () {
			      ...
			      return ( 
						     <select className="selectpicker "  key={this.props.name}   onChange={this.onChangeValue} multiple={this.props.multiple}  data-live-search="true" ref={this.props.name}  id="test"  >
						             {rows}
		         			 </select>
	         			 );
				    }
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement(SelectComponent, { multiple: false, url: '?controller=UserController&act=getUserByDept&depts=3,4', name: 'visitor' })
						),
						React.createElement(
							'td',
							null,
						...
						React.createElement(
							'td',
							null,
							React.createElement(SelectComponent, { multiple: false, url: '?controller=CustomerProcessController&act=getCustomerSuccessBackBacth&type=1', name: 'bacth_id' })
						),
						React.createElement(
							'td',
							null,
					...
					React.createElement('tr', { 'data-search-toggle': 'hide' }),
					React.createElement('tr', { 'data-search-toggle': 'hide' })
				)
			);
		}
	...
	var TableCommonRowComponent = React.createClass({
		...
		render: function render() {
			...
			this.props.indexs.forEach(function (index) {
				...
				if (index == "collect_voucher") {

					tds.push(React.createElement(
						'td',
						null,
						' ',
						React.createElement('img', { src: this.props.row[index], style: { height: '20px', width: '50px' } })
					));
				} else {
					tds.push(React.createElement(
						'td',
			...
			return React.createElement(
				'tr',
				{ onClick: this.onClickRow, 'data-id': this.props.row.rec_id, 'data-seller_id': this.props.row.seller_id, 'data-contract_id': this.props.row.contract_id, 'data-province': this.props.row.province_id },
	...
	var BodyTableComponent = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				'div',
				{ style: { width: 'auto', height: '380px', overflow: 'auto' }, border: '1' },
				React.createElement(
					'table',
					{ className: 'table table-hover table-bordered ', id: 'body_table_list', 'data-body': 'body_double' },
					React.createElement(
						'thead',
						null,
	...
	var SearchButton = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'a',
					{ href: '?controller=CustomerProcessController&act=customerBackEventList' },
					React.createElement(
						'button',
						null,
						'查看事件'
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#add_back_type_list' },
					'添加类型'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_back_chg_list' },
					'批量修改回访人'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#end_visit_list' },
					'结束回访'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#message_send_list' },
					'赠送试用短信'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_back_visit_chg' },
					'查看/修改联系人'
				)
			);
		}
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=CustomerProcessController&act=customerVisitProcessList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=CustomerProcessController&act=customerVisitProcessList&" + page_data);

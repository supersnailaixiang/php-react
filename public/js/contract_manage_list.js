...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["合同编号", "合同简称", "合同状态", "公司名称", "店铺名称", "合同金额", "服务费金额", "实际收款绩效", "应付绩效", "未付金额", "合同签署日期", "订购版本", "来源", "竞争对手", "是否定制", "是否独享", "销售1", "销售2", "创建人", "修改时间", "创建时间"];

	var indexs = ["contract_no", "contract_short", "contract_status", "company_name", "shop_name", "contract_amount", "server_fee", "perform_amount", "perform_amount2", "contract_no_paid", "contract_sign_time", "order_version", "source_name", "soft_name", "is_order", "is_share", "sale_user_name_1", "sale_user_name_2", "creator_name", "modified", "created"];
	...
	var operates = []; // 需要输入
	var page_turn = page;
	var url = "?controller=ContractController&act=getContractManageList";

	var tabs = [{ name: "合同明细", url: "?controller=ContractController&act=getContractDetailListByContractID" }, { name: '收款明细', url: "?controller=ContractController&act=getContractPaymentListByContractID" }, { name: '成本明细', url: "?controller=ContractController&act=getContractCostListByContractID" }, { name: "合同日志", url: "?controller=ContractController&act=contractLogList" }, { name: "工单", url: "?controller=ContractController&act=getContractCustomerWorkListByContractID" }, { name: "服务费变更记录", url: "?controller=ContractController&act=getContractServerChgListByContractID" }, { name: "日志", url: "?controller=CustomerProcessController&act=customerSellerLogList&type=contract_id" }];
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement('input', { type: 'text', name: 'contract_no' })
						),
						React.createElement(
							'td',
							null,
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement('input', { type: 'date', name: 'begin_sign_date' })
						),
						React.createElement(
							'td',
							null,
							React.createElement('input', { type: 'date', name: 'end_sign_date' })
						),
						React.createElement(
							'td',
							null,
	...
	var SearchButton = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#contract_list_reject' },
					'合同驳回'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', className: 'check_contract' },
					'查看合同'
				),
				'设置标记:',
				React.createElement(
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#contract_btn_chg' },
					'修改公司/店铺名称'
				)
			);
		}
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=ContractController&act=contractManageList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=ContractController&act=contractManageList&" + page_data);

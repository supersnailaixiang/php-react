...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["客户流转", "工单类型", "卖家账号", "实施工程师", "协助工程师", "实施监督", "销售1", "公司名称", "店铺名称", "初始密码", "软件版本", "省", "市", "地址", "下单时间", "分配时间", "联系客户时间", "上门时间", "离开现场时间", "提交结项时间", "结项时间"];

	var indexs = ["work_status", "work_type", "seller_account", "imp_user_name", "imp_user_name2", "imp_user_name3", "sale_man", "company_name", "shop_name", "begin_password", "order_name", "province_name", "city_name", "address", "work_order_time", "assign_work_time", "link_customer_time", "come_customer_time", "leave_scene_time", "submit_project_time", "end_project_time"]; //相当于sql的表头

	var table_rows = result;
	var user_flags = flags;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "查看客户信息", "class2": "customer_check", "target": "#customer_check_list", "name": "" }, { "title": "添加事件", "class2": "add_event", "target": "#add_event_list", "name": "" }, { "title": "添加任务", "class2": "add_task", "target": "#add_task_list", "name": "" }, { "title": "短信赠送", "class2": "message_send", "target": "#message_send_list", "name": "" }, { "title": "申请卖家账号", "class2": "apply_seller_account", "target": "#apply_seller_account_list", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=CustomerProcessController&act=getCustomerWorkManageList";

	var tabs = [{ name: "事件", url: "?controller=EventController&act=getImpEventListByWorkID" }, { name: "任务", url: "?controller=TaskController&act=getTaskListByWorkID" }, { name: "日志", url: "?controller=CustomerProcessController&act=customerWorkLogList" }];
	...
	var TableCommonRowComponent = React.createClass({
		...
		render: function render() {
			...
			this.props.indexs.forEach(function (index) {
				if (count == 0) {
					if (this.props.row['is_click'] == 1) {
						tds.push(React.createElement(
							'td',
							null,
							React.createElement(
								'button',
								{ className: 'customer_roll btn btn-primary' },
								this.props.row['work_status']
							)
						));
					} else {
						tds.push(React.createElement(
							...
							React.createElement(
								'button',
								{ disabled: true, className: 'customer_roll btn' },
								this.props.row['work_status']
							)
						));
					}
			...
			if (this.props.row.bg_color == null) {
				return React.createElement(
					'tr',
					{ onClick: this.onClickRow, 'data-id': this.props.row.rec_id, 'data-status': this.props.row.status, 'data-customer_id': this.props.row.customer_id, 'data-seller_id': this.props.row.seller_id, 'data-province_id': this.props.row.province_id },
				...
				return React.createElement(
					'tr',
					{ onClick: this.onClickRow, 'data-id': this.props.row.rec_id, 'data-status': this.props.row.status, 'data-customer_id': this.props.row.customer_id, 'data-seller_id': this.props.row.seller_id, 'data-province_id': this.props.row.province_id, style: { background: '#' + this.props.row.bg_color } },
					React.createElement(TdOperateComponent, { operates: this.props.operates }),
					tds
				);
			}
	...
	var BodyTableComponent = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				'div',
				{ id: 'table_div', style: { width: '2200px', height: '380px', overflow: 'auto' }, border: '1' },
				React.createElement(
					'table',
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement(SelectComponent, { multiple: true, url: '?controller=UserController&act=getUserByDept&depts=1', name: 'sales' })
						),
						React.createElement(
							'td',
							null,
						...
						React.createElement(
							'td',
							null,
							React.createElement(SelectComponent, { multiple: true, url: '?controller=UserController&act=getUserByDept&depts=3', name: 'imp_user_id' })
						),
						React.createElement(
							'td',
							null,
						...
						React.createElement(
							'td',
							null,
							React.createElement('select', { name: 'province_id', id: 'search_province' })
						)
					)
				)
			);
	...
	var SearchButton = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#imp_stop_list' },
					'实施暂停'
				),
				React.createElement(
					'button',
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_reassign_imp_list' },
					'二次分配实施'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_info_chg' },
					'修改信息'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_region_chg' },
					'修改大区'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#seller_account_chg' },
					'修改卖家账号/初始密码'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#add_tasks_list' },
					'批量添加任务'
				)
			);
		}
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=CustomerProcessController&act=customerWorkManageList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					var turn_url = "?controller=CustomerProcessController&act=customerWorkManageList&" + page_data;
					window.history.pushState(null, null, turn_url);

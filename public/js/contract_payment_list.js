...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["到款时间", "状态", "合同状态", "公司名称", "店铺名称", "到款账户", "汇款凭证", "合同编号", "到款金额", "合同金额", "未收金额", "开发票", "销售1", "销售2", "创建人", "所属大区", "创建时间"];

	var indexs = ["collect_time", "collect_status", "contract_status", "company_name", "shop_name", "collect_account", "collect_voucher", "contract_no", "collect_amount", "contract_amount", "contract_no_paid", "is_invoice", "sale_user_name1", "sale_user_name2", "creator_name", "region_name", "created"]; //相当于sql的表头

	var table_rows = result;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "收款", "class2": "btn_chg", "target": "#payment_collect_list", "name": "" }, { "title": "重新递交", "class2": "btn_submit", "target": "#payment_submit_list", "name": "" }, { "title": "查看合同", "class2": "btn_check_contract", "target": "", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=ContractController&act=getContractPaymentList";

	var tabs = [{ name: "收款日志", url: "?controller=ContractController&act=contractPaymentLogByID" }];
	...
	var TableCommonRowComponent = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				'tr',
				{ onClick: this.onClickRow, 'data-id': this.props.row.rec_id, 'data-contract_id': this.props.row.contract_id, 'data-province': this.props.row.province_id },
	...
	var BodyTableComponent = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'table',
					{ className: 'table table-hover table-bordered ', id: 'body_table_list', 'data-body': 'body' },
					React.createElement(
						'thead',
						null,
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=ContractController&act=getContractPaymentList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=ContractController&act=getContractPaymentList&" + page_data);

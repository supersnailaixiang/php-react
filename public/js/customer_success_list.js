...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["销售1", "销售2", "公司名称", "店铺名称", "状态", "所属行业", "所属类目", "所在省", "所在市", "店铺规模", "客户预算", "客户来源", "现用软件", "提供者", "创建者", "电话销售", "卖家账号", "初始密码", "修改时间", "成交时间"];

	var indexs = ["sale_user_name_1", "sale_user_name_2", "company_name", "shop_name", "status", "industry_name", "cate_name", "province_name", "city_name", "shop_scale", "customer_budget", "source_name", "soft_name", "supplier_name", "creator_name", "tel_name", "seller_account", "begin_password", "modified", "created"]; //相当于sql的表头

	var table_rows = result;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "修改客户", "class2": "btn_chg", "target": "#customer_info_chg", "name": "" }, { "title": "添加事件", "class2": "add_event", "target": "#add_event_list", "name": "" }, { "title": "添加任务", "class2": "add_task", "target": "#add_task_list", "name": "" }, { "title": "查看私密信息", "class2": "imporant_message", "target": "#check_important_message", "name": "" }, { "title": "修改成交客户", "class2": "modify_survey", "target": "#create_survey_list", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=CustomerProcessController&act=getCustomerSuccessList";

	var tabs = [{ name: "工单", url: "?controller=CustomerProcessController&act=getTabCustomerWorkList" }, { name: "日志", url: "?controller=CustomerProcessController&act=customerSellerLogList" }, { name: "事件", url: "?controller=EventController&act=getEventListBySellerID" }, { name: "任务", url: "?controller=TaskController&act=getTaskListBySellerID" }, { name: "便签", url: "?controller=CustomerController&act=getLabelListBySellerID" }];
	...
	var SelectComponent = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				'select',
				{ className: 'selectpicker ', key: this.props.name, onChange: this.onChangeValue, multiple: this.props.multiple, 'data-live-search': 'true', ref: this.props.name, id: this.props.id },
				rows
			);
		}
	});
	var SearchFormComponent = React.createClass({
		...
		render: function render() {
			return React.createElement(
				'form',
				{ ref: this.props.name, style: { padding: '5px' }, key: 'serach_form_list' },
				React.createElement(
					'table',
					null,
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement(SelectComponent, { multiple: true, url: '?controller=DictDataController&act=getDictData&type=6', name: 'industry_id', id: 'search_industry' })
						),
						React.createElement(
							'td',
							null,
						...
						React.createElement(
							'td',
							null,
							React.createElement('select', { name: 'cate_id', id: 'search_cate' })
						),
						React.createElement(
							'td',
							null,
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement(SelectComponent, { multiple: false, url: '?controller=UserController&act=getUserByDept&depts=1,2', name: 'supplier_id' })
						),
						React.createElement(
							'td',
							null,
					...
					React.createElement(
						...
						React.createElement(
							...
							React.createElement(
								'select',
								{ name: 'status', className: 'selectpicker', 'data-live-search': 'true' },
								React.createElement(
									'option',
									{ value: '99' },
									'全部'
	...
	var SearchButton = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_update_version_list' },
					'系统升级'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_open_stock_list' },
					'开从库/独享'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_buy_other_list' },
					'购买另一套'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_train_list' },
					'再次培训'
				),
				React.createElement(
					'button',
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_reject_list' },
					'单据驳回'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#apply_seller_account_list' },
					'申请卖家账号'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#user_expense_account_add_list' },
					'添加报销单'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#sale_adjust_percent_list' },
					'调整销售/绩效比例'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_refund_list' },
					'客户退款'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_api_warehouse_list' },
					'申请api对接/仓储对接'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_label_list' },
					'添加便签'
				)
			);
		}
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, " ?controller=CustomerProcessController&act=customerSuccessList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, " ?controller=CustomerProcessController&act=customerSuccessList&" + page_data);

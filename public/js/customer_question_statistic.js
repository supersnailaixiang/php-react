...
/***/ function(module, exports, __webpack_require__) {
	...
	var table_rows = result.result;
	var heads = result.heads;
	var indexs = result.indexs;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = []; // 需要输入
	//var page_turn = page;
	var url = "?controller=CustomerQuestionController&act=getCustomerQuestionStatistic";
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							...
							React.createElement(
								'select',
								{ name: 'statistic_type', className: 'selectpicker', 'data-live-search': 'true' },
								React.createElement(
									'option',
									{ value: '1' },
									'按产品统计'
	...
	var TableCommonRowComponent = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				'tr',
				{ onClick: this.onClickRow, 'data-id': this.props.row.rec_id },
				tds
			);
		}
	});
	var BodyTableComponent = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				'div',
				{ style: { width: 'auto', height: '400px', overflow: 'auto' }, border: '1' },
				React.createElement(
					'table',
	...
	var CustomerStatisticList = React.createClass({
		displayName: 'CustomerStatisticList',

		getInitialState: function getInitialState() {
			return { rows: table_rows, search_data: '', heads: heads, indexs: indexs };
		},
		onSearchSubmit: function onSearchSubmit(data) {
			...
			$.ajax({
				...
				success: function (result) {
					window.history.pushState(null, null, "?controller=CustomerQuestionController&act=CustomerQuestionStatistic&" + data);
					this.setState({ rows: result.result, heads: result.heads, indexs: result.indexs, search_data: data });
				}.bind(this),
				error: function (xhr, status, err) {
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				success: function (result) {
					...
					this.setState({ rows: result.result });
				}.bind(this),
				error: function (xhr, status, err) {
		...
		render: function render() {
			return React.createElement(
				...
				React.createElement(BodyTableComponent, { heads: this.state.heads, indexs: this.state.indexs, rows: this.state.rows, operates: operates })
			);
		}
	});
	ReactDOM.render(React.createElement(CustomerStatisticList, { url: url }), document.getElementById('content'));

...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["卖家账号", "公司名称", "店铺名称", "批次", "状态", "回访时间", "本次回访人", "回访备注", "实施大区", "销售大区", "成交时间"];

	var indexs = ["seller_account", "company_name", "shop_name", "bacth_name", "status", "visit_date", "visitor_name", "remark", "region_imp", "region_sale", "succ_time"]; //相当于sql的表头
	...
	var operates = []; // 需要输入
	var page_turn = page;
	var url = "?controller=CustomerProcessController&act=getCustomerBackBacthInfo";
	...
	var SelectComponent = React.createClass({
		...
		render: function render() {
			$.ajax({
				...
				success: function (data) {
					rows = [];
					var tmp_name = this.props.name;
					if (this.props.name == 'bacth_id') {
					...
					data.forEach(function (result) {
						if (tmp_name == 'bacth_id' && result.id == bacth_id && bacth_id > 0 && bacth_id != null) {
							rows.push(React.createElement(
								'option',
								{ key: result.id, selected: 'selected', value: result.id },
								result.name
							));
						} else {
							rows.push(React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement('select', { id: 'search_bacth_id', name: 'bacth_id' })
						),
						React.createElement(
							'td',
							null,
					...
					React.createElement(
						...
						React.createElement(
							'td',
							null,
							React.createElement('input', { type: 'date', name: 'begin_date1' }),
							React.createElement('br', null),
							React.createElement('input', { type: 'date', name: 'end_date1' })
						),
						React.createElement(
							'td',
							null,
						...
						React.createElement(
							...
							React.createElement(
								'select',
								{ name: 'imp_region_id', className: 'selectpicker', 'data-live-search': 'true' },
								React.createElement(
									'option',
									{ value: '0' },
									'全部'
	...
	var SearchButton = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#update_link_bacth_list' },
					'关联回访信息'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#customer_back_assign_list' },
					'批量分配回访人'
				)
			);
		}
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					window.history.pushState(null, null, "?controller=CustomerProcessController&act=customerBackBacthInfo&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=CustomerProcessController&act=customerBackBacthInfo&" + page_data);

...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["到款时间", "公司名称", "店铺名称", "到款账户", "汇款凭证", "合同编号", "到款金额", "合同金额", "未收金额", "开发票", "销售1", "销售2", "创建人", "所属大区", "创建时间"];

	var indexs = ["collect_time", "company_name", "shop_name", "collect_account", "collect_voucher", "contract_no", "collect_amount", "contract_amount", "contract_no_paid", "is_invoice", "sale_user_name1", "sale_user_name2", "creator_name", "region_name", "created"]; //相当于sql的表头

	var table_rows = result;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "收款", "class2": "btn_reject", "target": "#payment_collect_list", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=ContractController&act=getContractPaymentUrgeList";
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=ContractController&act=contractPaymentUrgeList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=ContractController&act=contractPaymentUrgeList&" + page_data);

...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["合同编号", "合同简称", "合同状态", "公司名称", "店铺名称", "合同金额", "服务费金额", "实际收款绩效", "应付绩效", "未付金额", "合同签署日期", "来源", "竞争对手", "是否定制", "是否独享", "销售1", "销售2", "创建人", "修改时间", "创建时间"];

	var indexs = ["contract_no", "contract_short", "contract_status", "company_name", "shop_name", "contract_amount", "server_fee", "perform_amount", "perform_amount2", "contract_no_paid", "contract_sign_time", "source_name", "soft_name", "is_order", "is_share", "sale_user_name_1", "sale_user_name_2", "creator_name", "modified", "created"];

	var table_rows = result;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "修改合同", "class2": "btn_chg", "target": "#contract_list_chg", "name": "" }, { "title": "重新递交", "class2": "btn_submit", "target": "", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=ContractController&act=getContractList";

	var tabs = [{ name: "合同明细", url: "?controller=ContractController&act=getContractDetailListByContractID" }, { name: '收款明细', url: "?controller=ContractController&act=getContractPaymentListByContractID" }, { name: '成本明细', url: "?controller=ContractController&act=getContractCostListByContractID" }, { name: "合同日志", url: "?controller=ContractController&act=contractLogList" }];
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							...
							React.createElement(
								'select',
								{ name: 'contract_type', className: 'selectpicker' },
								React.createElement(
									'option',
									{ value: '0' },
									'全部'
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=ContractController&act=contractList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=ContractController&act=contractList&" + page_data);

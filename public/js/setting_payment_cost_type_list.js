...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["类型", "名称", "是否内置", "修改时间", "创建时间"];

	var indexs = ["type", "name", "is_build", "modified", "created"]; //相当于sql的表头

	var table_rows = result;
	var user_flags = flags;
	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "修改", "class2": "btn_chg", "target": "#payment_cost_chg", "name": "" }, { "title": "删除", "class2": "btn_del", "target": "", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=SysSettingController&act=getSysPaymentAndCostTypeList";
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							...
							React.createElement(
								'select',
								{ name: 'type', className: 'selectpicker', 'data-live-search': 'true' },
								React.createElement(
									'option',
									{ value: 'cost' },
									'成本类型'
	...
	var SearchButton = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#type_add' },
					'添加类型'
				)
			);
		}
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, "?controller=SysSettingController&act=sysPaymentAndCostTypeList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, "?controller=SysSettingController&act=sysPaymentAndCostTypeList&" + page_data);
					this.setState({ rows: result.result, page: result.page });
				}.bind(this),
				error: function (xhr, status, err) {

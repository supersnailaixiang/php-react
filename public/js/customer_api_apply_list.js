...
/***/ function(module, exports, __webpack_require__) {
	...
	var heads = ["类型", "状态", "公司名称", "店铺名称", "卖家账号", "软件版本", "是否独享", "正式/测试", "客户负责人", "负责人联系方式", "负责人邮箱", "第三方系统名称", "对接类型", "对接客户类型", "对接业务", "审核备注", "收款金额", "实施工程师", "创建人", "创建时间"];

	var indexs = ["type", "status", "company_name", "shop_name", "seller_account", "order_name", "is_share", "is_formal", "link_man", "link_tel", "link_email", "other_system_name", "other_type", "api_customer_type", "other_business", "remark", "warehouse_amount", "imp_name", "user_name", "created"]; //相当于sql的表头

	var table_rows = result;

	var datas = ['rec_id'];
	//不需要输入，返回值
	var operates = [{ "title": "查看合同", "class2": "contract_check", "target": "", "name": "" }]; // 需要输入
	var page_turn = page;
	var url = "?controller=CustomerController&act=getCustomerApiApplyList";

	var tabs = [{ name: "操作日志", url: "?controller=CustomerController&act=customerApiWarehouseLogByID" }, { name: "事件", url: "?controller=EventController&act=getEventListBySellerID&type=1" }];
	...
	var TableCommonRowComponent = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				'tr',
				{ onClick: this.onClickRow, 'data-id': this.props.row.rec_id, 'data-seller_id': this.props.row.seller_id, 'data-type': this.props.row.doc_type },
	...
	var SearchButton = React.createClass({
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#api_warehouse_reject' },
					'驳回'
				),
				React.createElement(
					'button',
					{ 'data-toggle': 'modal', 'data-target': '#api_warehouse_agree' },
					'开通'
				)
			);
		}
	...
	var CustomerIntentionList = React.createClass({
		...
		onSearchSubmit: function onSearchSubmit(data) {
			$.ajax({
				...
				success: function (result) {
					...
					window.history.pushState(null, null, " ?controller=CustomerController&act=customerApiApplyList&" + data);
		...
		handleTablePage: function handleTablePage(page) {
			...
			$.ajax({
				...
				data: page_data,
				success: function (result) {
					window.history.pushState(null, null, " ?controller=CustomerController&act=customerApiApplyList&" + page_data);

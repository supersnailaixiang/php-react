...
/***/ function(module, exports, __webpack_require__) {
	...
	// type 控件类型
	// name 控件name
	// url type 是select 的时候获得数据的url
	//label_name 第一个label的名称
	// multiple type 是select 的时候是否多选。

	var React = __webpack_require__(1);
	var ReactDOM = __webpack_require__(38);
	var SearchFormComponent = __webpack_require__(168);
	...
	var data = [{ type: "input", name: "chen", url: "", label_name: "测试", multiple: 0, type_add: "text" }, { type: "input", name: "chen2", url: "", label_name: "测试2", multiple: 0, type_add: "text" }, { type: "select", name: "ai", url: "get_data.php", label_name: "测试", multiple: true, type_add: "" }]; // 所有的搜索框
	var heads = ["表头测试1", "表头测试2"];
	var indexs = ["test1", "test2"]; //相当于sql的表头
	var rows = [{ "test1": "chen", "test2": "ai" }, { "test1": "xiang", "test2": "success" }]; //不需要输入，返回值
	var operates = [{ "title": "test", "class2": "btn_chg", "target": "#myModal" }]; // 需要输入
	var page = { "total": 1, "cur_page": 1, "total_page": 1 };
	var id = "chen"; //需要输入

	var tabs = ["日志", "信息"];
	var TestWeb = React.createClass({
		displayName: 'TestWeb',
	...
	ReactDOM.render(React.createElement(TestWeb, null), document.getElementById('content'));
...
/***/ function(module, exports, __webpack_require__) {
	...
	//label_name 第一个label的名称
	// multiple type 是select 的时候是否多选。
	//type_add type 是input 的是type

	var React = __webpack_require__(1);
	var ReactDOM = __webpack_require__(38);
	React.createElement("link", { rel: "stylesheet", href: "public/css/crm.css" });
	var divs = [];
	var rows = [];
	   ...
	   */
			...
			$.ajax({
				...
				success: function (data) {
					...
					data.forEach(function (result) {
						...
						//	rows.push(<OptionComponent key={result.id} result={result} />);
		...
		render: function render() {
			...
			return React.createElement(
				...
				React.createElement(
					"select",
					{ className: "selectpicker ", onChange: this.onChangeValue, multiple: this.props.multiple, "data-live-search": "true", ref: this.props.name, id: "test" },
					rows
				)
			);
		}
			...
			this.props.data.forEach(function (result) {
				...
				if (result.type == "select") {
					divs.push(React.createElement(
						"td",
						{ key: result.label_name },
						result.label_name
					));
					divs.push(React.createElement(
						"td",
						null,
						React.createElement(SelectComponent, { key: result.name, name: result.name, url: result.url, multiple: result.multiple })
					));
				} else {
					divs.push(React.createElement(
						"td",
					...
					divs.push(React.createElement(
						"td",
						null,
						React.createElement("input", { type: result.type_add, name: result.name })
					));
				}
			});
			divs.push(React.createElement(
				"button",
				{ key: "btn_handle", onClick: this.handleSubmit },
				"查询"
			));
		},
		render: function render() {
